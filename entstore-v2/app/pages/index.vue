      ...
      <AppArticle :title="isMobile ? '企業情報・ニュースリリース' : 'ニュースリリース'" href="/news">
        <ul class="news">
          <li v-for="news in newsList" :key="news.id">
            ...
            <a :href="`/news/detail/${news.id}`" v-html="checkLengthTitle(news.title, 100).replaceAll('<br>', '')"></a>
          </li>
        </ul>
      ...
      <article id="bargainArea">
        ...
        <div class="mobile-box">
          <iframe id="chirashi-next-iframe"
            src="https://next.retailstudio.jp/entetsu-store/039/chirashi/iframe/?shop-id=0000" scrolling="no"
            frameborder="0" width="" height=""></iframe>
        ...
        <section v-for="(recipe, index) in recipeList" :key="index"
          ...
          <a target="_blank"
            ...
            <div class="title-recipe">
              <p class="category c_stats03" :style="{ 'background-color': categoryColor[recipe.category.toString()] }">
      ...
      <AppArticle class="d-none-des" title="サービス" href="/service">
        <div class="pa-2">
          <section v-for="(service, index) in services" :key="index" class="serviceInner">
            <a :href="service.to">
              <figure><img :src="$appendWebpFormat(service?.url)" :alt="service.imgAlt" class="fullImage d-block">
              </figure>
              <h2>{{ service.title }}</h2>
            </a>
          </section>
...
export default {
  ...
  async setup() {
    ...
    const { data: __d } = await useAsyncData(
      ...
      async () => {
        ...
        const [infoRes, sliderRes] = await Promise.all([
  ...
  methods: {
    ...
    async fetchOtherData() {
      try {
        // Nuxt 2: this.$nuxt.context.$microcms — Nuxt 4 khong con $nuxt.context.
        // Dung $microcms tiem toan cuc trong plugins/globals.ts.
